...
function paymentAmountColorClass(status: string): string {
  if (NO_MONEY_COMMITTED.has(status)) return 'text-[#64748B]' // gris: pendiente / cancelado / devuelto / congelado
...
export default function PatientPaymentsPage() {
  ...
  const { data: refundAccount } = useQuery({
    queryKey: ['patient', 'refund-account'],
    queryFn: patientsAPI.getMyRefundAccount,
    enabled: !!pendingRefunds && pendingRefunds.length > 0,
  })
  ...
  return (
    <DashboardLayout navItems={NAV} activeHref="/patient/payments" role="PATIENT">
      <div className="max-w-4xl">
        ...
        {hasPendingRefunds && !refundAccount && (
          <div className="mb-4 rounded-xl border border-[#F0C36D] bg-[#FDF6E7] p-3.5 flex items-center justify-between gap-3">
            <div>
        ...
        {hasPendingRefunds && refundAccount && !refundAccount.verified && (
          <div className="mb-4 rounded-xl border border-[#F0C36D] bg-[#FDF6E7] p-3 flex items-center gap-2">
            <span className="text-base">🔍</span>
            <p className="text-xs text-[#8A6116]">
              {t('Ya registraste tu cuenta para reembolsos — un administrador la está revisando antes de poder transferirte.')}
            </p>
          </div>
        )}

        {hasPendingRefunds && refundAccount?.verified && (
          <div className="mb-4 rounded-xl border border-[#BFD9EE] bg-[#EAF2FA] p-3 flex items-center gap-2">
            ...
            <p className="text-xs text-[#185FA5]">
              {pendingRefunds!.length === 1
                ? t('Tienes un reembolso aprobado — el equipo lo transferirá en breve a tu cuenta.')
                : `${t('Tienes')} ${pendingRefunds!.length} ${t('reembolsos aprobados — el equipo los transferirá en breve a tu cuenta.')}`}
            </p>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
              <div className="card py-3 text-center">
                ...
                <p className="text-xs text-[#475569] mt-0.5">{t('Total pagado')}</p>
              </div>
              <div className="card py-3 text-center">
                <p className="text-xl font-bold text-[#185FA5]">{stats?.consultas_pagadas ?? 0}</p>
                <p className="text-xs text-[#475569] mt-0.5">{t('Consultas pagadas')}</p>
              </div>
              <div className="card py-3 text-center">
                <p className="text-xl font-bold text-[#854F0B]">Bs. {stats?.total_pendiente.toFixed(2) ?? '0.00'}</p>
                <p className="text-xs text-[#475569] mt-0.5">{t('QR pendiente')}</p>
              </div>
              <div className="card py-3 text-center">
                <p className="text-xl font-bold text-[#475569]">Bs. {stats?.total_reembolsado.toFixed(2) ?? '0.00'}</p>
                <p className="text-xs text-[#475569] mt-0.5">{t('Reembolsado')}</p>
              </div>
            </div>
                        ...
                        {isOpen && (
                          <div className="mt-3 ml-[52px] bg-[#F5F6FA] rounded-lg p-3 space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              <div>
                                <p className="text-[#64748B]">{t('Monto pagado')}</p>
                              ...
                              {p.paid_at && (
                                <div>
                                  <p className="text-[#64748B]">{t('Fecha de pago confirmado')}</p>
                              ...
                              {p.bank_tx_id && (
                                <div>
                                  <p className="text-[#64748B]">{t('N° de transacción')}</p>
                                  <p className="text-[#3C4257]">{p.bank_tx_id}</p>
                                </div>
                              )}
                              {p.released_at && (
                                <div>
                                  <p className="text-[#64748B]">{t('Fecha de liberación al profesional')}</p>
                            ...
                            {p.disputed_at && (
                              <div className="pt-2 border-t border-[#DDE1EE] grid grid-cols-2 gap-3 text-xs">
                                ...
                                {p.dispute_reason && (
                                  <div className="col-span-2">
                                    <p className="text-[#64748B]">{t('Tu motivo')}</p>
