...
// Sección de Perfil: cuenta bancaria PERMANENTE para recibir
// reembolsos — espejo exacto de la sección "Cuenta bancaria" del
// profesional (ver src/app/professional/profile/page.tsx). El paciente
// la carga una sola vez; a partir de ahí, cualquier reembolso futuro
// entra directo a la cola de "listos para pagar" del admin en cuanto la
// verifica, sin tener que completar nada de nuevo. Si el paciente no
// tiene cuenta bancaria, el equipo administrativo lo contacta aparte
// para coordinar otra forma de pago (ver el aviso que se dispara desde
// app/services/refund_payout.py cuando le aprueban un reembolso sin
// cuenta registrada). Ver PUT /patients/me/refund-account.
...
export function RefundAccountSection() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => patientsAPI.updateMyRefundAccount({
  ...
  return (
    <div className="card">
      ...
      <p className="text-xs text-[#475569] mb-1">
        {t('Si alguna vez te corresponde un reembolso (por ejemplo, una cita cancelada), lo transferimos a esta cuenta. Cárgala una sola vez.')}
      </p>
      <p className="text-xs text-[#64748B] mb-3">
        {t('Si no tienes cuenta bancaria, no hace falta que completes esto: el equipo administrativo se pondrá en contacto contigo para coordinar otra forma de pago cuando te corresponda un reembolso.')}
      </p>
      ...
      {myAccount && (
        <div className="mb-4 p-3 rounded-lg bg-[#F5F6FA] border border-[#DDE1EE]">
          <div className="flex items-center justify-between mb-1">
            <p className="text-sm font-semibold text-[#141820]">{myAccount.bank_name}</p>
            {myAccount.verified ? (
      ...
      <p className="text-xs font-semibold text-[#141820] mb-2">
        {myAccount ? t('Cambiar cuenta bancaria') : t('Registrar cuenta bancaria')}
      </p>
      ...
      <div className="space-y-3">
        ...
        >
          {saveMutation.isPending ? t('Guardando...') : t('Guardar cuenta bancaria')}
        </button>
      </div>
    </div>
  )
