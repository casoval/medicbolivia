...
def generate_generic_invitation_pdf() -> bytes:
    """
    Variante genérica de la invitación, pensada para entrega en persona
    (el admin la imprime en tamaño carta y la entrega él mismo en el
    consultorio) en vez del envío individual por WhatsApp que llevó al
    baneo. Diferencias respecto a generate_invitation_pdf():
      - Saludo genérico "Estimado/a Doctor/a," (no hay un lead puntual
        detrás, así que no hay nombre que insertar).
      - No menciona WhatsApp en ningún lado — como es el propio médico
        quien decide escribir después de leer el papel, no queremos
        insinuar un canal de contacto no solicitado. En su lugar invita
        a visitar la página, donde encontrará los números de contacto.
      - Se mantiene el correo electrónico como vía de contacto directa.
    """
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        ...
        title="Invitación MedicBolivia",
        author="MedicBolivia",
        subject="Invitación general a unirse a MedicBolivia",
    ...
    elements.append(Paragraph("Estimado/a Doctor/a,", greeting_style))
