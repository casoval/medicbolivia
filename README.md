...
- ✅ **Gemini Live API — llamadas de voz en tiempo real con Medi**
- ✅ WebSocket directo al navegador — baja latencia, sin backend intermediario
- ✅ Medi saluda automáticamente al conectar — no espera que el paciente hable primero
- ✅ Tono de llamada tipo teléfono mientras conecta
- ✅ Banner animado — distingue si Medi está hablando o escuchando
- ✅ Barge-in — el paciente puede interrumpir a Medi mientras habla
- ✅ Detección automática de especialidad → busca médicos en el backend sin que el paciente escriba nada
- ✅ Despedida indica que los médicos recomendados aparecen en el chat
- ✅ AudioWorklet (sin ScriptProcessorNode deprecated)
- ✅ Scheduler de audio continuo con `AudioContext.currentTime` — sin cortes entre chunks
- ✅ VAD de Gemini con alta sensibilidad — responde en ~300ms de silencio
...
### Usuarios de prueba
- Admin:       teléfono `70000000` — contraseña en el gestor de secretos local, NO en este archivo
- Paciente:    teléfono `69625434` (creado durante pruebas locales)

> ⚠️ Nunca vuelvas a escribir contraseñas reales acá, aunque sean "de prueba". Este repo es público.
> Si necesitás documentarlas para vos, usá un archivo local sin trackear (ej. `NOTES.local.md`, ya en `.gitignore`) o un gestor de contraseñas.
...
Medi es el agente de orientación médica de MedicBolivia. Tiene dos modos de interacción:

### Modo texto / voz WhatsApp (backend)
...
### Modo llamada en tiempo real — Gemini Live API ⭐ NUEVO
Llamada de voz bidireccional en tiempo real directamente desde el navegador.

**Modelo:** `gemini-3.1-flash-live-preview` (el único activo en v1beta actualmente)
**Voz de Medi:** `Charon` (Gemini Native Audio)
**Protocolo:** WebSocket directo del browser a `generativelanguage.googleapis.com`
**Archivo:** `frontend/src/app/patient/agent/geminiLive.ts`

```
Paciente pulsa "Llamar a Medi"
        ↓
Tono de llamada (ringback 440Hz + 480Hz, Web Audio API)
WS abre + getUserMedia en paralelo (Promise.all)
        ↓
setupComplete → Medi saluda sola via realtime_input.text
        ↓
Conversación bidireccional en tiempo real
  • Mic → AudioWorklet → PCM16 → base64 → WS → Gemini
  • Gemini → PCM24 → AudioContext scheduler continuo → altavoz
        ↓
Medi detecta especialidad → backend busca médicos automáticamente
        ↓
Médicos aparecen en el chat sin que el paciente escriba nada
        ↓
Paciente cuelga → mensaje de despedida con link al chat
```

**Características técnicas del Live:**

| Aspecto | Implementación |
|---------|---------------|
| Captura de mic | `AudioWorklet` inline (Blob URL, sin archivo externo) |
| Buffer del mic | 512 muestras @ 16kHz = 32ms por chunk |
| Reproducción | Scheduler con `AudioContext.currentTime` — sin gaps entre chunks |
| Barge-in | Detección RMS local > 0.015 → cierra AudioContext → interrumpe Medi |
| VAD | `END_SENSITIVITY_HIGH`, `silenceDurationMs: 300` — responde en 300ms |
| Echo cancel | `echoCancellation`, `noiseSuppression`, `autoGainControl` en getUserMedia |
...
### Costo estimado Gemini Live
- Gemini Live API es gratuita en el tier free (con límites de rate)
- Plan de pago: ~$0.35 por hora de audio de entrada, ~$1.50 por hora de audio de salida
...
| `src/app/patient/agent/page.tsx` | Grabación de voz (mantener presionado). Reproductor audio estilo WhatsApp. Botón "Llamar a Medi". Banner animado Medi hablando/escuchando. Callback `onRecommendation` para mostrar médicos detectados por voz. |
| `src/app/patient/agent/geminiLive.ts` | **Nuevo archivo.** Gestor completo de Gemini Live API fuera de React. WebSocket, AudioWorklet, scheduler de audio, barge-in, ringtone, VAD, detección de especialidad. |
...
Usar el script `backend/create_admin.py`, que pide el teléfono y la contraseña
de forma interactiva (nunca hardcodear la contraseña acá ni en el script):
```bash
cd /var/www/medicbolivia/backend
source venv/bin/activate
...
### Cambiar la contraseña del admin existente
```bash
cd /var/www/medicbolivia/backend
source venv/bin/activate
...
| **Agente IA (llamada viva)** | **Gemini Live API — gemini-3.1-flash-live-preview** |
...
│       │   │   ├── dashboard/
│       │   │   ├── agent/
│       │   │   │   ├── page.tsx        # Chat + botón llamada + banner animado
│       │   │   │   └── geminiLive.ts   # ⭐ Gestor Gemini Live API
│       │   │   ├── search/
│       │   │   ├── waiting-room/
│       │   │   ├── history/
│       │   │   └── video/
...
- [x] ~~Llamadas telefónicas con Vapi.ai~~ → reemplazado por Gemini Live API directo ✅
- [ ] Ephemeral tokens para Gemini Live en producción (no exponer API key en el cliente)
